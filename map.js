...
$.when(
  ...
  for (let index = 0; index < amountOfDays + 5; index++) {
    ...
    if (index < amountOfDays)
      dateArray[index] =
        tempDate.getMonth() + 1 + "/" + tempDate.getDate() + "/20";
    datePredictArray[index] =
      tempDate.getMonth() + 1 + "/" + tempDate.getDate() + "/20";
  }
  ...
  for (let index = 0; index < dateArray.length; index++) {
    confirmedArray[index] = getTotalSum(
      confirmedData[0].features,
      dateArray[index]
    );
    deathArray[index] = getTotalSum(deathsData[0].features, dateArray[index]);
    recoveredArray[index] = getTotalSum(
      recoveredData[0].features,
      dateArray[index]
    );
  }
  /* ------------------      PREDICTION      ----------------------- */
  ...
  var newLegendClickHandler = function(e, legendItem) {
    ...
    if (index > 4) {
      ...
      if (ci.getDatasetMeta(legendItem.datasetIndex).hidden === null) {
        ci.getDatasetMeta(legendItem.datasetIndex).hidden = !ci.data.datasets[
          index
        ].hidden;
...
function getList(data, name) {
  ...
  merged.rows.sort(compare).map(item => {
    casesList.push(
      '<span id="' + name + 'List">' + item.value + "</span> " + item.key
    );
  });
  merged.rows.sort(compare).map(item => {
